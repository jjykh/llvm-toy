...
static void myexit(void) { exit(1); }
...
static void buildIR(State& state) {
  ...
  LValue result = output.buildSelect(
      output.buildICmp(LLVMIntNE, indicator, output.constIntPtr(0)), add, val);
...
static void mydispIndirect(void) { printf("%s.\n", __FUNCTION__); }

static void mydispDirect(void) { printf("%s.\n", __FUNCTION__); }

static void mydispAssist(void) { printf("%s.\n", __FUNCTION__); }

inline static uint8_t rexAMode_R__wrk(unsigned gregEnc3210,
                                      unsigned eregEnc3210) {
...
static inline unsigned iregEnc3210(unsigned in) { return in; }
...
inline static uint8_t* doAMode_R__wrk(uint8_t* p, unsigned gregEnc3210,
                                      unsigned eregEnc3210) {
...
static void patchProloge(void*, uint8_t* start, uint8_t* end) {
  uint8_t* p = start;
  *p++ = rexAMode_R(jit::RBP, jit::RDI);
  *p++ = 0x89;
  p = doAMode_R(p, jit::RBP, jit::RDI);
