...
RegisterSet StackMaps::Record::liveOutsSet() const {
  ...
  for (unsigned i = liveOuts.size(); i--;) {
    ...
    // FIXME: Either assert that size is not greater than sizeof(pointer), or
    // actually save the high bits of registers.
...
StackMaps::RecordMap StackMaps::computeRecordMap() const {
  ...
  for (unsigned i = records.size(); i--;)
    result
        .insert(std::make_pair(records[i].patchpointID, std::vector<Record>()))
