...
static inline LType structType(LContext context, LType* elementTypes,
...
static inline LType structType(LContext context, LType element1,
                               PackingMode packing = NotPacked) {
  return structType(context, &element1, 1, packing);
}
static inline LType structType(LContext context, LType element1, LType element2,
                               PackingMode packing = NotPacked) {
...
static inline LType functionType(LType returnType, const LType* paramTypes,
                                 unsigned paramCount, Variadicity variadicity) {
  return LLVMFunctionType(returnType, const_cast<LType*>(paramTypes),
                          paramCount, variadicity == Variadic);
}
template <typename VectorType>
inline LType functionType(LType returnType, const VectorType& vector,
...
static inline LType functionType(LType returnType, LType param1,
                                 Variadicity variadicity = NotVariadic) {
  return functionType(returnType, &param1, 1, variadicity);
}
static inline LType functionType(LType returnType, LType param1, LType param2,
                                 Variadicity variadicity = NotVariadic) {
...
static inline void setMetadata(LValue instruction, unsigned kind,
                               LValue metadata) {
...
static inline LValue addExternFunction(LModule module, const char* name,
                                       LType type) {
...
static inline LValue constInt(LType type, unsigned long long value,
...
static inline LBasicBlock appendBasicBlock(LContext context, LValue function,
                                           const char* name = "") {
...
static inline void addIncoming(LValue phi, const LValue* values,
...
static inline LValue buildStructGEP(LBuilder builder, LValue pointer,
                                    unsigned idx) {
  return LLVMBuildStructGEP(builder, pointer, idx, "");
}
static inline LValue buildStore(LBuilder builder, LValue value,
                                LValue pointer) {
...
static inline LValue buildICmp(LBuilder builder, LIntPredicate cond,
                               LValue left, LValue right) {
  return LLVMBuildICmp(builder, cond, left, right, "");
}
static inline LValue buildFCmp(LBuilder builder, LRealPredicate cond,
                               LValue left, LValue right) {
  return LLVMBuildFCmp(builder, cond, left, right, "");
}
static inline LValue buildInsertElement(LBuilder builder, LValue vector,
                                        LValue element, LValue index) {
  return LLVMBuildInsertElement(builder, vector, element, index, "");
}

enum SynchronizationScope { SingleThread, CrossThread };
static inline LValue buildFence(LBuilder builder, LAtomicOrdering ordering,
...
static inline LValue buildCall(LBuilder builder, LValue function,
                               const LValue* args, unsigned numArgs) {
  return LLVMBuildCall(builder, function, const_cast<LValue*>(args), numArgs,
                       "");
}
template <typename VectorType>
inline LValue buildCall(LBuilder builder, LValue function,
...
static inline void setInstructionCallingConvention(
    LValue instruction, LCallConv callingConvention) {
  LLVMSetInstructionCallConv(instruction, callingConvention);
}
static inline LValue buildExtractValue(LBuilder builder, LValue aggVal,
                                       unsigned index) {
  return LLVMBuildExtractValue(builder, aggVal, index, "");
}
static inline LValue buildSelect(LBuilder builder, LValue condition,
                                 LValue taken, LValue notTaken) {
...
static inline LValue buildCondBr(LBuilder builder, LValue condition,
                                 LBasicBlock taken, LBasicBlock notTaken) {
  return LLVMBuildCondBr(builder, condition, taken, notTaken);
}
static inline LValue buildSwitch(LBuilder builder, LValue value,
                                 LBasicBlock fallThrough, unsigned numCases) {
  return LLVMBuildSwitch(builder, value, fallThrough, numCases);
}
static inline void addCase(LValue switchInst, LValue value,
                           LBasicBlock target) {
...
static inline LValue constInlineAsm(LType Ty, const char* AsmString,
